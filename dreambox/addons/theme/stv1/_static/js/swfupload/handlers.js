...
function fileQueueError(file, errorCode, message)  {
    try {
        ...
        case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
        	dreambox.alert("上传了过多文件，" + (message === 0 ? "上传文件限制" : "你可以选择 " + (message > 1 ? "上传" + message + " 个文件." : "一个文件.")));
            return;
        case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
           dreambox.alert("上传的文件请小于"+this.settings.file_size_limit);
        ...
        case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
        	dreambox.alert("待上传的文件请选择"+this.settings.file_types+"类型文件");
            this.debug("Error Code: Invalid File Type, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
            return;
        default:
        	dreambox.alert("上传过程中发生异常，请刷新后稍后再上传");
