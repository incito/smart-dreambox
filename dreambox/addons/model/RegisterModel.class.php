...
class RegisterModel extends Model {
	...
	public function isValidPassword($pwd, $repwd) {
		...
		$length = strlen($pwd);
		if(strpos($pwd, ' ')!==false){
			$this->_error = '密码不能包含空格';			// 密码太短了，最少6位
			$res = false;
		} else if($length < 6) {
			...
			$this->_error = '密码与确认密码不一致';		// 新密码与确认密码不一致
			$res = false;
		}
	 ...
	 */
	public function sendActivationEmail($uid, $node ='register_active',$code) {
		$map['uid'] = $uid;
...
// 			$this->_user_model->cleanCache($uid);
			$this->_user_model->updateCache($uid,array('is_active'=>'1'));
		}
	 ...
	 * @return string 激活码
	 */
	public function getActivationCodeByDetail($login,$password,$login_salt){
		return md5($login.$password.$login_salt);
	}
