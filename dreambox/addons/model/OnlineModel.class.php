...
class OnlineModel{
	...
	public function dostatus() {
	    //PC端的访问统计
	    $pc_tables['online'] = "ts_online";
	    $pc_tables['stats'] = "ts_online_stats";
	    $pc_tables['logs'] = "ts_online_logs";
	    $pc_tables['logs_bak'] = "ts_online_logs_bak";
	    $this->stat($pc_tables);

	    //微信端的访问统计
	    $wx_tables['online'] = "ts_online_wechat";
	    $wx_tables['stats'] = "ts_online_wechat_stats";
	    $wx_tables['logs'] = "ts_online_wechat_logs";
	    $wx_tables['logs_bak'] = "ts_online_wechat_logs_bak";
	    $this->stat($wx_tables);
	}
	 ...
	 * @return boolean
	 */
	private function stat($tables){

	    // 获取最大ID
	    $sql = 'SELECT MAX(id) AS max_id FROM '.$tables['logs'].' WHERE statsed = 0';
	    $max_id = $this->odb->query($sql);
	    $max_id = @$max_id['0']['max_id'];
	    if(empty($max_id)) {
	        return false;
	    } else {
	        $sql = "UPDATE ".$tables['logs']." SET statsed = 1 WHERE id < {$max_id}";
	    ...
	    if(!empty($userData)) {
    	    ...
    	    foreach($upData as $k=>$v) {
        	    $sql = "SELECT id FROM ".$tables['stats']." WHERE day = '{$k}'";
        	    $issetRow  = $this->odb->query($sql);
        	    if(empty($issetRow)) {
            	    $sql = "INSERT INTO ".$tables['stats']." (`day`,`total_users`,`total_guests`,`total_pageviews`)
            	            VALUES ('{$k}','{$v['total_users']}','{$v['total_guests']}','{$v['total_pageviews']}')";
        	    } else {
            	    $sql = " UPDATE ".$tables['stats']."
        ...
        $sql = "INSERT INTO ".$tables['logs_bak']." SELECT * FROM `".$tables['logs']."` WHERE day <='".date('Y-m-d', strtotime('-1 day'))."'";
                $this->odb->execute($sql);
                // 删除logs表中今天之前的数据删除
                $sql = " DELETE FROM `".$tables['logs']."` WHERE day <='".date('Y-m-d', strtotime('-1 day'))."'";
                // 统计结束
		$this->odb->execute($sql);
	}
	...
	public function checkOnline() {
	    ...
	    $wx_tables['logs'] = "ts_online_wechat_logs";
	    $wx_tables['logs_bak'] = "ts_online_wechat_logs_bak";
	    $this->check($wx_tables);
	}
	
	/**
	 * 检查当前在线的用户
	 */
	private function check($tables){
		$startTime = time() - $this->stats_step;
		$day = date('Y-m-d');
		// 今日统计数据
		$sql = "SELECT * FROM ".$tables['stats']." WHERE day ='{$day}'";
		$dayData =  $this->odb->query($sql);

		if(!empty($dayData)) {
			// 在线注册用户
			$sql = "SELECT COUNT(1) AS pu FROM ".$tables['online']." WHERE uid !=0 AND activeTime  >= {$startTime}";
			...
			// 在线游客
			$sql = "SELECT COUNT(ip) AS pu FROM ".$tables['online']." WHERE uid = 0 AND activeTime >= {$startTime}";
			...
			if(!empty($set)) {
				$sql = " UPDATE ".$tables['stats']." SET ".implode(',', $set)." WHERE day = '{$day}'";
				$this->odb->execute($sql);
			}
		}
