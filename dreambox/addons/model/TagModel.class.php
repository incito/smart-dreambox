...
class TagModel extends Model {
	...
	public function getHotTags($limit = 15, $expire = 3600) {
		...
		if (($hot_tag_list = S ( $cache_id )) === false) {
			...
			$hot_tag_ids = D ( 'app_tag' )->field ( "`tag_id`, COUNT(`tag_id`) AS `count`" )->group ( "`tag_id`" )->order ( '`count` DESC' )->limit ( $limit )->findAll ();
			// 获得标签文字
	...
	/**
	 * 获取当前学期的所有课程,去重复得到课程标签
	 */
	public function getSelectCourseTag($userId) {
		$termModel = D ( 'Term', 'dreambox' );
		$term = $termModel->getCurrentTerm ( $userId );
		
		if (empty ( $term )) {
			return array ();
		}
		return $this->db->query ( 'SELECT t.tag_id as id,c.class_name as name,t.tag_hid hid FROM ts_db_select_course s INNER JOIN ts_db_course c ON s.term_id=' . $term ['id'] . ' AND s.course_id=c.id INNER JOIN ts_tag t ON c.class_name=t.name GROUP BY s.course_id LIMIT 0,5' );
	}
	/**
	 * 给博文添加标签
	 */
	public function addBlogTag($uid, $blog_id, $tags, $course_id) {
		if (! is_array ( $tags )) {
			$tags = explode ( '-', $tags );
		}
		$sql = "DELETE FROM ts_blog_tag WHERE blog_id=" . $blog_id;
		M ()->execute ( $sql );
		foreach ( $tags as $tag ) {
			if ($tag == 0) {
				continue;
			}
			$sql = "INSERT INTO ts_blog_tag (blog_id,tag_id) VALUES ('.$blog_id.','.$tag.')";
			M ()->execute ( $sql );
			// 添加关系表
			// 如果是课程标签,则需要添加
			if (! empty ( $course_id )) {
	 ...
	 * 获取当前学期的所有课程,去重复得到课程标签
	 */
	public function getAllCourseTag($userId) {
		return $this->db->query ( 'SELECT t.tag_id as id,t.name,t.tag_hid hid FROM ts_db_course c INNER JOIN ts_tag t ON c.class_name=t.name AND t.tag_hid=2' );
	}
	/**
	 * 获取文章热门标签
	 */
	public function getHotBlogTag($limit) {
		return $this->db->query ( 'SELECT b.tag_id as id,t.name as name,t.tag_hid hid FROM  ts_blog_tag b INNER JOIN ts_tag t ON b.tag_id=t.tag_id GROUP BY b.tag_id ORDER BY count(*) desc limit 0,' . $limit );
	}
	/**
	 * 根据标签id获取标签
	 */
	public function getTagById($tagId) {
		return $this->where ( 'tag_id=' . $tagId )->find ();
	}
}