...
class MessageModel extends Model {
     ...
     * 获取私信列表（平铺）-分页型
     */
	public function getMessageList($uid, $type = 1){
		$uid = intval($uid);
		$type = is_array($type) ? ' IN ('.implode(',', $type).')' : "={$type}";
		$list=M('message_content')->join('as mc left join ts_message_member mb on mb.list_id=mc.list_id AND `mb`.`is_del` = 0 left join `ts_message_list` AS `li` ON `mb`.`list_id`=`li`.`list_id` ')
								->where('`mb`.`member_uid`='.$uid.' AND `li`.`type` '.$type.' AND  mc.is_del=0  AND mb.message_num > 0')
								->field('mc.message_id,mc.list_id,mc.from_uid,mc.content,mc.attach_ids,mc.mtime')
								->order('mc.mtime desc')
								->findPage();
		return $list;
	}
    ...
    private function _parseMessageList(&$list, $current_uid) {
        foreach ($list as &$v) {
            ...
            $user_info= model('User')->getUserInfo($v['last_message']['from_uid']);
            $v['last_message']['user_info']['uname'] =$user_info['uname'];
            $v['last_message']['user_info']['avatar_url']=$user_info['avatar_small'];
            unset($user_info);
        }
    }
    ...
    public function getMessageByListId($list_id, $uid, $since_id = null, $max_id = null, $count = 20) {
        ...
        foreach($res['data'] as $r_d_k => $r_d_v) {
        	$user_info=model('User')->getUserInfo($r_d_v['from_uid']);
            $res['data'][$r_d_k]['user_info']['uname'] = $user_info['uname'];
            $res['data'][$r_d_k]['user_info']['avatar_url'] = $user_info['avatar_small'];
            $res['data'][$r_d_k]['mtime']=date('Y年m月d日 H:i:s', $res['data'][$r_d_k]['mtime']);
            // 处理附件信息
            $attachType = '';
            $attachArr = array();
