...
// ***************************************************ts2.XX  应用移动新增函数
	...
	public function searchFollows ($key, $type, $limit = 20, $uid, $gid) {
		...
		if ($type === 'following') {
			...
			$map['a.`fid`'] = $uid;
			$map['b.`uname`'] = array('LIKE', '%'.$key.'%');
			$list = D()->table('`'.C('DB_PREFIX').'user_visited` AS a LEFT JOIN `'.C('DB_PREFIX').'user` AS b ON a.`uid` = b.`uid`')
					   ->field('a.`uid` AS `fid`')
					   ->where($map)
					   ->findPage($limit);
		}
	 ...
	 * 最近访客列表
	 */
	public function getVisitorList($uid, $limit){
	    $list = M('user_visited')->where("fid=$uid")->order('ctime DESC')->findpage($limit);
	    foreach ($list['data'] as $key=>$value){
	        $uid = $value['uid'];
	        $fid = $value['fid'];
	        $list['data'][$key]['uid'] = $fid;
