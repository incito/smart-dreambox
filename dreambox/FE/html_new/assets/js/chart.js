var charts={
	...
	chartConfig: function (container, option) { //加载Echarts配置文件,container:为页面要渲染图表的容器，option为已经初始化好的图表类型的option配置
        var chart_path = "plug/echarts/echarts"; //配置图表请求路径
        var map_path = "plug/echarts/echarts-map";//配置地图的请求路径
...
    renderMap: function (option) {//渲染地图
		...
		function (ec) {
			...
			option.chart.on('mapSelected', function (param){//点击地图
			    ...
			    if (mt == 'china') {
			        ...
			        option.option.tooltip.formatter = '3213123123{b}';
			        param1.province = charts.mapType[charts.curIndx];
			    }
			    else {
			    	//选中市区
			        option.option.tooltip.formatter = '123123{b}';
			        param1.province = mt;
	...
	searchCon:function(param,isScroll){
		...
		charts.isAjax = true;
		$.ajax({
            url: 'http://192.168.30.111/dreambox/index.php?app=dreambox&mod=DreamCenter&act=filterArea',
            data: param,
            type:"get",
            dataType:'jsonp',
	...
	insertContent:function(data,isSearch,isScroll){
		...
		for (var i = 0; i < schoolInfo.length; i++) {
			...
			str += '<h6><a href="U(\'public/Profile/index\')&uid='+schoolInfo[i].id+'">'+schoolInfo[i].name+'</a></h6>';
