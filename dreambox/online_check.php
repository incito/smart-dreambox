...
if($_GET['action']=='trace'){
    $online_tablename = "ts_online";
    $logs_tablename = "ts_online_logs";
    //来源于微信端的访问
    if($_GET['from']=='wechat'){
        $sql = "SELECT u.uid,u.uname FROM ts_user_bind b, ts_user u WHERE b.uid = u.uid AND bindId='".$_GET['openid']."' LIMIT 1";
        $result = $db->query($sql);
        if($result){
            $uid = $result[0]['uid'];
        ...
        $online_tablename = "ts_online_wechat";
        $logs_tablename = "ts_online_wechat_logs";
    }
    
	/* ===================================== step 1 record track ========================================== */
		
	$sql	=	"INSERT INTO ".$logs_tablename." 
	...
	//刷新用户在线时间
		...
		$sql	=	"SELECT uid FROM ".$online_tablename." ".$where;
       
		$result	=	$db->query("$sql");
		//如果没有记录.添加记录.
		if($result){

			$sql	=	"UPDATE ".$online_tablename." SET activeTime=$cTime,ip='$ip' ".$where;
			$result	=	$db->execute("$sql");	
		}else{

			$sql	=	"INSERT INTO ".$online_tablename." (uid,uname,app,ip,agent,activeTime) VALUES ('$uid','{$uname}','$app','$ip','$agent',$cTime);";
			$result	=	$db->execute("$sql");
		}
