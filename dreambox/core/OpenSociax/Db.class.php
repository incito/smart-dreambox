...
class Db extends Think
{
    ...
    protected function multiConnect($master=false) {
    	...
    	return $this->doConnect($db_config,$r);
    }
    
    /**
     * 连接数据库方法
     * @paramboolean $master 主服务器
     * @access public
    ...
    public function connect($master=false) {
    	if(1 == C('DB_DEPLOY_TYPE')){
            ...
            $config =   $this->config;
            $this->_linkID =$this->doConnect($config);
        }
    }
	
    protected function doConnect($config='',$linkNum=0){
...
//             else
    ...
    public function queryFromMaster($str='',$method='',$p='',$p2='') {
    	$this->connect(true);
    	if ( !$this->_linkID ) return false;
    	if ( $str != '' ) $this->queryStr = $str;
    	//释放前次的查询结果
