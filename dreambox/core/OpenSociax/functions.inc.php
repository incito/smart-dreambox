...
function cookie($name,$value='',$option=null) {
    ...
    if (''===$value){
        ...
        if (is_null($value)) {
            setcookie($name,'',time()-80000000,$config['path'],$config['domain']);
...
function L($key,$data = array()){
    $key = strtoupper($key);
     if(!isset($GLOBALS['_lang'][$key])){
     	  ...
     	  !is_array($notValveForKey)&&$notValveForKey = array();
...
function getShort1($str, $length = 40, $ext = '') {
    $str    =   htmlspecialchars($str);
    $str    =   strip_tags($str);
    $str    =   htmlspecialchars_decode($str);
    $strlenth   =   0;
...
/**
 * 计算从stime算起，etime所在的周数
 * @param unknown $stime 基准时间 unixtime 
 * @param unknown $etime 结束时间 unixtime
 * @return number
 */
function week_count($stime,$etime){
	$smondaytime=this_monday($stime);
	$num=floor( ($etime - $smondaytime) / (7 * 24 * 60 * 60) );
	// 当前为第几周
	return  $num+1 ;
}
/**
 * 
	这个星期的星期一
 	@param $timestamp ，某个星期的某一个时间戳，默认为当前时间
	@param is_return_timestamp ,是否返回时间戳，否则返回时间格式
 */
 ...
 * @param unknown $uid 当前用户UID
 */
function isBindablePhone($phone,$uid){
	if(empty($phone)){
		return false;
	}
	$map['phone']=$phone;
	$uid&&$map['uid']=array('neq'=>$uid);
	return !M('user_verified_info')->where($map)->find();
}
function log_wx($content) {
	// 打开日志
	$log = fopen ( "saestor://data/log.txt", "a+" );
	fwrite ( $log, date ( '[c]' ) . '  ' . $content );
	fclose ( $log );
}