...
class Db
{
    ...
    public function execute($str='') {
        ...
        $this->debug();
        if ( false === $result) {echo var_dump($this->error());
            if ( $this->debug )
                throw_exception($this->error());
            else
                return false;
   ...
   /**
     ...
     * 查询被禁用的用户id
     */
    public function queryDisabledUserIds()
    {
        //禁用的用户ID
        $sql = 'SELECT value FROM `ts_system_data` WHERE `list` = \'audit\' and `key`=\'banneduids\' LIMIT 1';
        $result = $this->query($sql);
        $disableIds = array();
        if(!empty($result)){
            $strIds = $result[0]['value'];
            $start = strpos($strIds, '"');
            $strIds = substr($strIds, $start + 1);
            $strIds = strstr($strIds, '"', true);
            $disableIds = explode("|", $strIds);
        }
        return $disableIds;
    }
     ...
     * 查询2.0的用户与学校的关系信息
     */
    public function queryGroupMember()
    {
        $sql = 'SELECT uid, gid, level FROM `ts_group_member`';
     ...
     * 查询用户id与email的对应关系
     */
    public function queryUserEmail()
    {
        $sql = 'SELECT uid, email FROM `ts_user`';
        $result = $this->query($sql);
        foreach ($result as $user){
            $uEmail[$user['email']] = $user['uid'];
        }
        return $uEmail;
    }
