...
class TestAction extends Action{
	...
	public function cut(){
		getThumbImage("./data/uploads/2012/0604/19/4fcc9b2f67d34.jpg",'300','auto',false);
		echo '<img src="./data/uploads/2012/0604/19/4fcc9b2f67d34_300_auto.jpg">';
		echo 11;
	}
	...
	public function upFacePath () {
		...
		if (empty($list)) {
			...
			foreach ($list as $uid) {
				...
				//增加各种尺寸的图片
				$small = UPLOAD_PATH.'/avatar/'.$uid.'/small.jpg';
				$res = copy($small, $path.'/original_30_30.jpg');
				$middle = UPLOAD_PATH.'/avatar/'.$uid.'/middle.jpg';
				$res = copy($middle, $path.'/original_50_50.jpg');
// 				$res = copy($oldPath, $path.'/original_100_100.jpg');
				$res = copy($oldPath, $path.'/original_200_200.jpg');
	 ...
	 * 计算文章的的第一张图片
	 */
	public function convertBlog(){
// 	    $data = M('blog')->select();
// 	    foreach ($data as $blog)
// 	    {
// 	        $content = safe($blog['content']);
// 	        $images = matchImages($content);
//             $images[0] && $cover = $images[0];
//             if(!empty($cover)){
//                 M('blog')->setField('cover', $cover);
//             }
// 	    }
	}
}