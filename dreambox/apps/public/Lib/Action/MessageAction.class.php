...
//         $realname = M('UserVerified')->getRealname($this->uid);
        $user = M( 'User' )->getUserInfo ( $this->uid );
        $uname = $user['uname'];
        $this->assign('realname', $uname);
        $this->display();
    }
	...
	public function index() {
		...
		foreach($list['data'] as &$v){
			$v['messages'] = $dao->getMessageByListId(intval($v[list_id]), $this->mid, '-1', 0);
		}
		// 设置信息已读(在右上角提示去掉),
	...
	public function indexList() {
		$dao = model('Message');
		$list = $dao->getMessageList($this->mid, array(MessageModel::ONE_ON_ONE_CHAT, MessageModel::MULTIPLAYER_CHAT));
		$user_dao=model('User');
		foreach ($list['data'] as &$v){
			$user_info=$user_dao->getUserInfo($v['from_uid']);
			$v['user_info']['uname'] = $user_info['uname'];
			$v['user_info']['avatar_url'] = $user_info['avatar_small'];
			$v['mtime']=date('Y年m月d日 H:i:s', $v['mtime']);			
		}
		// 设置信息已读(在右上角提示去掉),
...
// echo 1;
	...
	public function notify() {
		...
		M ( 'UserCount' )->resetUserCount ( $this->mid, 'unread_notify');
