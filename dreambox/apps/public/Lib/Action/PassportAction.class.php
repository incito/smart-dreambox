...
class PassportAction extends Action 
{
	...
	public function login(){
		// 添加样式
		$this->appCssList [] = 'login_base.css';
	 ...
	 * @return void
	 */
	public function loginBak(){
		// 添加样式
		$this->appCssList[] = 'login_bak.css';
		if(model('Passport')->isLogged()){
			U('public/Index/index','',true);
		}
	...
	public function doLogin() {
		$login 		= addslashes(trim($_POST['login_email']));
	 ...
	 * 通过Email找回密码
	 */
	public function doFindPasswordByEmailAjax() {
		$_POST["email"]	= t($_REQUEST["email"]);
		...
		if(!$user) {
			$this->ajaxReturn(null,'找不到该邮箱注册信息','0');
		}
	
		$result = $this->_sendPasswordEmail($user);
		if($result){
			$this->ajaxReturn(null,'发送成功，请注意查收邮件','1');
		}else{
			$this->ajaxReturn(null,'操作失败，请重试','0');
		}
	}
	/**
	 * 通过Email找回密码
	...
	public function doResetPassword() {
		$code = t($_POST['code']);
		$user_info = $this->_checkResetPasswordCode($code,true);

		$password = $_POST['password'];
		$repassword = $_POST['repassword'];
		$reg_dao=model('Register');
		if(!$reg_dao->isValidPassword($password, $repassword)){
			$this->ajaxReturn(null,$reg_dao->getLastError(),0);
		}
	 ...
	 * @return void
	 */
	private function _checkResetPasswordCode($code,$isAjax=false) {
		$map['code'] = $code;
		$map['is_used'] = 0;
		$uid = D('find_password')->where($map)->getField('uid');
		if(!$uid){
			if($isAjax){
				$this->ajaxReturn(null,'重置密码链接已失效，请重新找回',0);
			}else{
// 				$this->assign('jumpUrl',U('public/Passport/findPassword'));
		...
		if (!$user_info) {
			if($isAjax){
				...
				$this->redirect = U('public/Index/index');
			}
		}
