...
class AttachAction extends Action{
	...
	public function download(){
		...
		$attach	=	model('Attach')->where("attach_id={$aid} AND uid={$uid}")->find();
		//$attach	=	model('Xattach')->where("id='$aid'")->find();
		if(!$attach){
			$this->error(L('attach_noexist'));
		}
		//下载函数
// 		require_cache(SITE_PATH.'/addons/library/Http.class.php');
	...
	public function kissy(){
		...
		if(is_array($info['info'])){
			$image_url	=	SITE_URL.'/data/upload/'.$info['info'][0]['savepath'].$info['info'][0]['savename'];
		}
