...
class RegisterAction extends Action
{
	...
	protected function _initialize()
	{
		...
		if($this->mid > 0 && !in_array(ACTION_NAME, array('changeActivationEmail', 'activate', 'isEmailAvailable', 'isValidVerify'))) {
			...
			$GLOBALS['ts']['user']['is_audit'] == 1 && $GLOBALS['ts']['user']['is_active'] == 0&& ACTION_NAME != 'waitForActivation'&&ACTION_NAME!='resendActivationEmail' && U('public/Register/waitForActivation', array('uid'=>$this->mid), true);
		}
		// 登录后，将不显示注册页面
	...
	public function doStep1(){	
		...
		//是否是重新注册
		$oldUid=intval($_POST['uid']);
		if(!empty($oldUid)){
			$oldUser=$this->_user_model->where('uid='.$oldUid.' and is_active=0')->field('email,uname')->find();
		}
		$is_valid=true;
		//检查验证码
// 		if (md5(strtoupper($_POST['verify'])) != $_SESSION['verify']) {
...
// 		}
		...
		if(!$this->_register_model->isValidName($uname,$oldUser?$oldUser['uname']:null)) {
// 			$this->error($this->_register_model->getLastError());
			$ret['uname']=$this->_register_model->getLastError();
			$is_valid=false;
		}

		if(!$this->_register_model->isValidEmail($email,$oldUser?$oldUser['email']:null)) {
// 			$this->error($this->_register_model->getLastError());
			$ret['email']=$this->_register_model->getLastError();	
			$is_valid=false;			
		}

		if(!$this->_register_model->isValidPassword($password, $repassword)){
// 			$this->error($this->_register_model->getLastError());
			$ret['password']=$this->_register_model->getLastError();
			$is_valid=false;
		}
		//校验未通过，返回
		if(!$is_valid){
			$this->ajaxReturn($ret,null,'0');
			return;
		}
		...
		//重新注册时update后返回
		if($oldUser){
			$save=$this->_user_model->where('uid='.$oldUid.' and is_active=0')->save($map);
// 			$this->_user_model->cleanCache($oldUid);
			//重新注册后刷新缓存
			$this->_user_model->updateCache($oldUid,$map);
			$ret['uid']=$oldUid;
			if(!$save){
				$ret['email']='注册失败';
			}else{
				$code=$this->_register_model->getActivationCodeByDetail($map['login'],$map['password'],$map['login_salt']);
				$this->_register_model->sendActivationEmail($oldUid,'register_active',$code);
			}
			$this->ajaxReturn($ret,null,$save?'1':'0');
		}
		$uid = $this->_user_model->add($map);
		if($uid) {
			$ret['uid']=$uid;
			...
			if($this->_config['register_audit']) {
				...
				if(!$isActive){
					$this->_register_model->sendActivationEmail($uid);
// 					$this->redirect('public/Register/waitForActivation', array('uid' => $uid));
					$ret['goUrl']=U('public/Register/waitForActivation', array('uid' => $uid));
					$this->ajaxReturn($ret,null,'1');
				}else{
					D('Passport')->loginLocal($email,$password);
// 					$this->assign('jumpUrl', U('public/Passport/login'));
					$ret['jumpUrl']=U('public/Index/index');
					$ret['goUrl']=THEME_PATH.'/success.html';
					$this->ajaxReturn($ret,null,'1');
// 					$this->success('恭喜您，注册成功');
				}
			}
	...
	public function waitForActivation() {
		...
		if($user_info) {
			if($user_info['is_audit'] == '0') {
				...
				// 已激活则回到首页
				exit(U('public/Index/index',array(),true));				
			}
		} else {
			// 注册
...
// 			$this->redirect('public/Index/index');
	...
	public function toLoginEmail(){
		...
		$splits=split('@', $email);
		if(count($splits)>1){
			$emails=split('\.',$splits['1']);
			$email_map=C('email_map');
			if(array_key_exists($emails[0],$email_map ) ){
				header('location:http://'.$email_map[$emails[0]]);
			}else{
				header('location:http://mail.'.$splits[1]);
			}
		}else{
			$this->error('不正确的邮箱地址');
		}
