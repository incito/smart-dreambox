...
class IndexAction extends Action {
    ...
    public function addEvent() {
    	if (! CheckPermission ( 'event_normal', 'lanuch_event' )) {
    ...
    public function doAddEvent() {
        //$this->_createLimit($this->mid);
		if(!$GLOBALS['ts']['isSystemAdmin'] && intval($_POST['type']) == 3){
			$this->error("非法操作");
		}
		...
		foreach($cover['info'] as $key => $val){
			if($val['key'] != 'cover'){
				$appendix_id[$key] = $val['id'];
			}
		}

		$map['appendix_id'] = implode(',',$appendix_id);
    ...
    public function show_achievement() {
        ...
        $this->event->setMid( $this->mid );
        if($event_result = $this->event->getEventContent( $id,$uid )) {

        	$event_achievement = M('event_achievement');
			$result = $event_achievement -> where("eid = $id") -> find();

			if($result['appendix_id']){
				$appendix_id = explode(',',$result['appendix_id']);
				foreach($appendix_id as $key => $val){
					$result['appendix_list'][$key] = $this->event->getCover2($val);
				}
			}
    ...
    public function integral() {
		if($GLOBALS['ts']['isSystemAdmin']){
			...
			switch( $_GET['action'] ) {
				case "is":
					$map['action'] = array( 'in',"'joinIn','admin'" );
					$map['status2'] = 1;
					$map['status'] = 1;
					break;
				case "no":
			...
			//取得成员列表
			$map['eventId'] = $id;
			$result = $this->event->getMember($map,$event['uid'],'cTime asc');

			//已确认参加人数
			$this->assign('is_count',M('event_user') ->where("eventId='$id' and action in('joinIn') and status=1 and status2=1") -> count());
			$this->assign('no_count',M('event_user') ->where("eventId='$id' and action in('joinIn') and status=1 and status2=0") -> count());
			$this->assign( $result );
			$this->setTitle('成员列表');
			$this->display();
		}else{
     ...
     * @return void
     */
    public function doIsAgreeAction() {
        $data['id']      = intval( $_POST['id'] );
        $data['eventId'] = intval( $_POST['eventId'] );
        $data['uid']     = intval( $_POST['uid'] );
		
		if(intval($_POST['status2']) == '0'){
			$map['status2'] = 1; 
		}elseif(intval($_POST['status2']) == '1'){
			$map['status2'] = 0; 
		}else{
			echo 3;
            return;
		...
		$res = M('event_user') ->where($data) -> save($map);
		if($res){
			echo 1;
            return;
		}else{
	...
	public function doAllAgree(){
		if($GLOBALS['ts']['isSystemAdmin']){
			...
			if($_POST){
				...
				$map['uid'] = array("in",implode(',',$checkbox_arr));
				$data['status2'] = '1';
	...
	public function doCredit(){
		if($GLOBALS['ts']['isSystemAdmin']){
			...
			if($_POST){
				$credit = $_POST['credit'];
				$uname = M('User') -> where("uid=".$event['uid']) -> getField('uname');
				if(!preg_match('/^\d+$/',$_POST['credit'])) $this->error("金币必须是整数");
				
				$event_user = M("event_user");
				$event_user_list = $event_user->where("eventId='$id' and action<>'admin' and status=1 and status2=1 and is_credit=0")->field("uid")->select();
				
				$credit_user = M('credit_user');
				$score_log = M('score_log');
				foreach($event_user_list as $key => $val){
					$score = $credit_user -> where("uid=".$val['uid']) -> getField('score');
					mysql_query("SET AUTOCOMMIT=0");
					mysql_query("BEGIN");
					$credit_res = $credit_user -> where("uid=".$val['uid']) -> save(array("score"=>($score+$credit)));

					$score_data['uid'] = $val['uid'];
					$score_data['float'] = $credit;
					$score_data['scoreSource'] = "管理员发放：参加活动获取的金币";
					$score_data['addTime'] = time();
					$score_log_res = $score_log->add($score_data);

					$event_user_res = $event_user->where("eventId='$id' and action<>'admin' and status=1 and status2=1 and uid=".$val['uid'])->save(array("is_credit"=>1));

					$str = "感谢您参加由'".$uname."'账号发起的'".$event['title']."'活动，由于您已经按照活动要求完成了相关任务，特此奖励您".$credit."梦想盒子金币";
					

					$notify_data = array('title' => '活动金币发放', 'content' => $str);
					$res = service('Notify')->sendIn($val['uid'], 'admin_notification', $notify_data);

					if(!$credit_res || !$score_log_res || !$event_user_res){
    ...
    public function doEditEvent() {
		...
		foreach($_FILES as $key => $val){
			if($key != 'cover' && $val['size']>0){
				$appendix_res = true;
				break;
			}
		}
		if($appendix_res){
			$appendix_info = X('Xattach')->upload('event');
			if(!$appendix_info['status']){
				$this->error($appendix_info['info']);
			}
			
			foreach($appendix_info['info'] as $key => $val){
				if($val['key'] != 'cover'){
					$appendix_id[$key] = $val['id'];
				}
			}
			
			if($_POST['appendix_id']){
				$new_arr = array_merge($appendix_id,$_POST['appendix_id']);
			}else{
				$new_arr = $appendix_id;
			}
			$map['appendix_id'] = implode(',',$new_arr);
		}else{
			$map['appendix_id'] = implode(',',$_POST['appendix_id']);
		}
        //处理省份，市，区
        list( $opts['province'],$opts['city'],$opts['area'] ) = explode( " ",$_POST['city']);;
   ...
   /*public function upload_single_pic() {
     ...
     * @return void
     */
    public function doAddAchievement() {
		
		$id = intval($_POST['id']);
		$event_achievement = M('event_achievement');
		$achievement_id = $event_achievement -> where("eid = $id") -> getField("id");
		$event_count = D('Event') -> where("id=$id and uid=".$this->mid) -> count();
		if($event_count > 0 && !$achievement_id){
			$map['achievement_content']    = h($_POST['explain']);
			$map['eid'] = $id;
			$map['create_time'] = time();
			if(strlen(text($map['achievement_content'])) < 10){
				$this->error('介绍不得小于10个字符');
			}
			//得到上传的图片
			$config     =   getConfig();
			...
			if( $addId = M('event_achievement')->add($map)) {
				$this->assign('jumpUrl',U('event/Index/show_achievement',array('id'=>$id,'uid'=>$this->mid)));
				$this->success('添加成功');
			}else{
				$this->error('添加失败');
			}
		}elseif($event_count > 0 && $achievement_id){
			$map['achievement_content']    = h($_POST['explain']);
			if(strlen(text($map['achievement_content'])) < 10){
				$this->error('介绍不得小于10个字符');
			}
			...
			if( (M('event_achievement')->where("eid=$id and id=$achievement_id")->save($map)) !== false) {
				$this->assign('jumpUrl',U('event/Index/show_achievement',array('id'=>$id,'uid'=>$this->mid)));
				$this->success('保存成功');
			}else{
				$this->error('保存失败');
