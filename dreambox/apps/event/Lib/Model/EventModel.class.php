...
class EventModel extends BaseModel {
	...
	public function getEventContent($eventId, $uid) {
		...
		$result ['lc'] = 5000000 < $result ['limitCount'] ? "5000000" : $result ['limitCount'];
		...
		if ($result ['appendix_id']) {
			...
			foreach ( $appendix_id as $key => $val ) {
				$result ['appendix_list'] [$key] = $this->getCover2 ( $val );
			}
		}
		return $result;
	}
	...
	function getCover($coverId, $width = 80, $height = 80) {
		$cover = D ( 'Attach' )->field ( 'id,savepath,savename' )->find ( $coverId );
		if ($cover) {
	...
	// 获取活动附件存储地址
	function getCover2($coverId) {
		$cover = D ( 'Attach' )->field ( 'id,savepath,savename,name' )->find ( $coverId );
		$cover ['url'] = SITE_URL . "/index.php?app=home&mod=Attach&act=download&id=$coverId&uid=" . $this->uid;
	...
	public function getMember($map, $uid, $order = '') {
		$user = self::factoryModel ( 'user' );
		$result = $user->getUserList ( $map, 20, true, $order );
		$data = $result ['data'];
		// 修正成员状态
	...
	public function get_hot_list() {
		$result = M ()->query ( "select * from ts_event where is_hot=1 order by rtime desc" );
		// 将属性追加
	 ...
	 */
	/*
	 * public function getConfig( $index ){ $config = $this->config->$index; return $config; }
	 */
	 ...
	 * @return void
	 */
	public function doIsHot2($map, $act) {
		if (empty ( $map )) {
			throw new ThinkException ( "不允许空条件操作数据库" );
		}
		...
		switch ($act) {
			case "recommend" : // 推荐
				$data ['is_hot'] = 1;
				$data ['rtime'] = time ();
				$result = $this->where ( $map_opts )->save ( $data );
				break;
			case "cancel" : // 取消推荐
				$data ['is_hot'] = 0;
				$data ['rtime'] = 0;
	 ...
	 * 获取课程中心活动
	 */
	public function getCourseCenterEvent($num) {
		$type = M ( 'event_type' )->where ( "name='课程中心'" )->field ( 'id' )->find ();
		$time = strtotime ( date ( 'Y-m-d' ) );
		return $this->where ( 'type=' . $type ['id'] . ' and deadline>=' . $time )->limit ( $num )->select ();
	}
}
?>