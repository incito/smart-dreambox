...
class IndexAction extends Action {
	...
	public function my() {
		...
		$list = isset ( $_GET ['outline'] ) ? $outline->getList ( $this->mid ) : 		// 草稿箱
	...
	public function personal() {
		...
		// 归档数据
		$url = isset ( $GET ['cateId'] ) ? 'Index/personal/uid/' . $uid . '/cateId/' . intval ( $_GET ['cateId'] ) : 'Index/personal/uid/' . $uid;
	...
	public function doDeleteblog() {
		...
		if (false != $result) {
			// 更新文章数
			model ( 'UserData' )->setUid ( $this->mid )->updateKey ( 'blog_count', 1, false );
	...
	public function doAddBlog() {
		...
		// 检查是否为空
		if (! matchImages ( $_POST ['content'] ) && trim ( $content ) == '') {
			$this->error ( "博文内容不能为空" );
		}
		...
		if ($add) {
			// 添加文章数
			model ( 'UserData' )->setUid ( $this->mid )->updateKey ( 'blog_count', 1, true );
			
			X ( 'Credit' )->setUserCredit ( $this->mid, 'add_blog' );
			$html = '【' . text ( $data ['title'] ) . '】' . getShort ( $content, 80 ) . U ( 'blog/Index/show', array (
					'id' => $add,
					'mid' => $this->mid 
			) );
			$image = $images [0] ? $images [0] : false;
			$this->ajaxData = array (
					'url' => U ( 'blog/Index/show', array (
							'id' => $add,
							'mid' => $this->mid 
					) ),
					'id' => $add,
			...
			// 为保证只有一份草稿
			if ($_POST ['delTemp']) {
				$conditon ['uid'] = $this->mid;
				$conditon ['status'] = 3;
				$this->blog->where ( $conditon )->delete ();
			}
			// 添加标签
			model ( 'Tag' )->addBlogTag ( $this->mid, $blogId, $_POST ['tags'], $_POST ['course_id'] );
			$res ['id'] = $blogId;
			$res ['mid'] = $this->mid;
	...
	public function doUpdate() {
		...
		if ($save) {
			...
			// 添加标签
			model ( 'Tag' )->addBlogTag ( $this->mid, $id, $_POST ['tags'], $_POST ['course_id'] );
			$res ['id'] = $id;
			$res ['mid'] = intval ( $_POST ['uid'] );
			$this->ajaxReturn ( $res, '发布成功', 1 );
		} else {
			$this->error ( "修改失败" );
		}
	}
	private function __getPost() {
		...
		$data ['title'] = ! empty ( $_POST ['title'] ) ? text ( $_POST ['title'] ) : "";
		$data ['private'] = intval ( $_POST ['private'] );
		$data ['canableComment'] = intval ( t ( $_POST ['cc'] ) );
		$data ['status'] = empty ( $_POST ['status'] ) ? 1 : $_POST ['status'];
	...
	public function isCategoryExist() {
		$name = t ( $_POST ['name'] );
		$list = M ( "BlogCategory" )->where ( array (
				'name' => $name,
				'uid' => $this->mid 
		) )->getField ( "name" );
		if ($list) {
