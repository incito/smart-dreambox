...
class BlogModel extends BaseModel {
	 ...
	 * @return unknown
	 */
	public function getBlogListForBox($mid, $map, $beginNum = 0, $pageSize = 5) {
		// //转载 的博文
		// $condition['t.mid'] = $map['uid'];
		// $condition['t.type'] = 1;
		// $condition['b.status'] = 1;
		// $ids = $this->join('as b inner join ts_blog_comment as t on t.blog_id = b.id')->where($condition)->field('b.*')->select();
		// foreach ($ids as $id){
		// $bids[] = $id['blog_id'];
		// }
		// $where['uid'] = $map['uid'];
		// $where['id'] = array('in', $bids);
		// $where['_logic'] = 'or';
		// $map['_complex'] = $where;
		// $map['status'] = 1;
		
		// $where = $this->_queryBlogWhere ( $map );
		// $result = $this->query ( 'SELECT u.uname as uname,b.* FROM ts_user u INNER JOIN ts_blog b on u.uid=b.uid' . $where . 'order by cTime desc limit ' . $beginNum . ',' . $pageSize );
		$result = $this->join ( 'left join ts_user as u on ts_blog.uid = u.uid' )->where ( $map )->order ( 'cTime desc' )->field ( 'u.uname,ts_blog.*' )->limit ( $beginNum . ',' . $pageSize )->select ();
		foreach ( $result as & $blog ) {
			
			$this->_queryCount ( $blog, $mid );
			
			$this->_queryBlogTags ( $blog );
	 ...
	 * @param string $map        	
	 */
	public function getBlogCountForBox($map) {
		// $where = $this->_queryBlogWhere ( $map );
		// $result = $this->query ( 'select count(1) as count from ts_blog as b' . $where );
		// return $result [0] ['count'];
		return $this->where ( $map )->field ( 'id' )->count ();
	}
	/**
	 * 查询博文的sql条件，包含转载的博文
	 *
	 * @return string
	 */
	private function _queryBlogWhere($map) {
		$uid = $map ['uid'];
		$begin = $map ['begin'];
		$end = $map ['end'];
		return ' where (b.uid in( ' . $uid . ') or b.id in (select  t.blog_id from ts_blog_comment as t where t.mid in( ' . $uid . ') and t.type = 1 )) and b.status =1 and (b.cTime >= ' . $begin . ' and b.cTime < ' . $end . ') ';
	}
	 ...
	 * @param unknown $blog        	
	 */
	private function _queryCount(& $blog, $mid) {
		$condition ['mid'] = $mid;
		$condition ['blog_id'] = $blog ['id'];
		// 是否已经转载
		$condition ['type'] = 1;
		$blog ['shared'] = M ( 'blog_comment' )->where ( $condition )->count ();
		// 是否已经喜欢
		$condition ['type'] = 2;
		$blog ['liked'] = M ( 'blog_comment' )->where ( $condition )->count ();
	}
	 ...
	 * @param unknown $blog        	
	 */
	private function _queryBlogTags(&$blog) {
		$tags = M ()->query ( 'SELECT t.tag_id id,t.name FROM ts_blog_tag b INNER JOIN ts_tag t ON b.tag_id=t.tag_id AND b.blog_id=' . $blog ['id'] );
		$blog ['tags'] = $tags;
	}
	 ...
	 * @param unknown $blog        	
	 */
	private function _convert(&$blog) {
		$ctime = $blog ['cTime'];
		$shortResult = getBlogShort1 ( $blog ['content'], 200 );
		$blog ['content_short'] = $shortResult ['output'] == null ? "" : $shortResult ['output'];
		$blog ['hasMore'] = $shortResult ['hasMore'];
// 		if (stripos ( $blog ['content'], '<img ' ) === false && (mb_strlen ( $blog ['content'], 'utf8' ) < 200 || mb_strlen ( $blog ['content_short'], 'utf8' ) < 200)) {
// 			$blog ['content_short'] = $blog ['content'];
// 			$blog ['hasMore'] = false;
// 		}
		$blog ['cDate'] = date ( 'm-d', $ctime );
		$blog ['cYear'] = date ( 'Y', $ctime );
		$blog ['cTime'] = date ( "Y-m-d H:i:s", $ctime );
		$blog ['mTime'] = date ( "Y-m-d H:i:s", empty ( $blog ['mTime'] ) ? $ctime : $blog ['mTime'] );
	}
	
	/**
	 * 得到博客的所有年份
	 */
	public function getYearsOfBlog($uid, $is_school) {
		$years [] = date ( 'Y', time () );
		if ($is_school) {
			$uvModel = M ( 'UserVerified' );
			$map ['sid'] = $uvModel->where ( 'uid=' . $uid )->getField ( 'sid' );
			$map ['verified'] = '1';
			$uids = $uvModel->where ( $map )->getField ( 'id,uid' );
			$uids && $uids = array_values ( $uids );
			$bmap ['uid'] = array (
					'in',
					$uids 
			);
			$uids && $data = $this->where ( $bmap )->order ( 'cTime desc' )->field ( 'cTime' )->select ();
			$uids && $birthday = M ( 'school' )->where ( 'id=' . $map ['sid'] )->getField ( 'applytime' );
			$birthday = date ( 'Y-m-d', $birthday );
		} else {
			$map ['status'] = 1;
			$map ['uid'] = $uid;
			$data = $this->where ( $map )->order ( 'cTime desc' )->field ( 'cTime' )->select ();
		}
		
		foreach ( $data as $blog ) {
			$years [] = date ( 'Y', $blog ['cTime'] );
		}
		// 去重
	...
	public function getBlogCategory($uid, $cateId) {
		...
		if (isset ( $cateId ) && ! self::_checkCategory ( $cateId, $list ))
			return false;
		return $result;
	}
	...
	// 获取当前用户的暂存文章
	public function getTempBlog($uid) {
		$map ['uid'] = $uid;
		$map ['status'] = 3;
		return $this->where ( $map )->find ();
	}
	 ...
	 * 获取首页博客
	 */
	public function getIndexBlog($mid, $type, $start, $num = 5, $searchKey) {
		if (empty ( $searchKey )) {
			// 全部动态
			if ($type == 1) {
				$sql = 'SELECT u.uname as uname,b.id,b.uid,b.cover,b.title,b.content,b.cTime,b.mTime,b.commentCount,b.republishCount,b.likeCount,b.terminal FROM ts_user u INNER JOIN  ts_blog b on u.uid=b.uid AND b.status=1 ORDER BY b.cTime desc LIMIT ' . $start . ',' . $num;
			} else if ($type == 2) { // 推荐
				$sql = 'SELECT u.uname as uname,b.id,b.uid,b.cover,b.title,b.content,b.cTime,b.mTime,b.commentCount,b.republishCount,b.likeCount,b.terminal FROM ts_user u INNER JOIN  ts_blog b on u.uid=b.uid AND b.status=1 ORDER BY b.cTime desc LIMIT ' . $start . ',' . $num;
			} else { // 我喜欢的
				$sql = 'SELECT u.uname as uname,b.id,b.uid,b.cover,b.title,b.content,b.cTime,b.mTime,b.commentCount,b.republishCount,b.likeCount,b.terminal FROM ts_user u INNER JOIN  ts_blog b on u.uid=b.uid AND b.status=1 INNER JOIN ts_blog_comment c ON c.blog_id=b.id AND c.type=2 AND c.mid=' . $mid . ' ORDER BY b.cTime desc LIMIT ' . $start . ',' . $num;
			}
		} else {
			// 关键字
			$searchKey = "'%" . $searchKey . "%'";
			...
			// 全部动态
			if ($type == 1 || $type == 2) {
				$sql = 'SELECT  u.uname as uname,b.id,b.uid,b.cover,b.title,b.content,b.cTime,b.mTime,b.commentCount,b.republishCount,b.likeCount,b.terminal FROM ts_blog b INNER JOIN  ts_user u  ON u.uid=b.uid AND b.status=1 where (b.title LIKE ' . $searchKey . ' OR b.content LIKE  ' . $searchKey . ' OR u.uname LIKE  ' . $searchKey . ' ) ORDER BY b.cTime DESC limit ' . $start . ',' . $num;
				
				$sqlTag = 'SELECT u.uname as uname,b.id,b.uid,b.cover,b.title,b.content,b.cTime,b.mTime,b.commentCount,b.republishCount,b.likeCount,b.terminal FROM  ts_blog b LEFT JOIN ts_user u ON u.uid=b.uid AND b.status=1 INNER JOIN ts_blog_tag bt INNER JOIN ts_tag t ON bt.tag_id =t.tag_id AND b.id=bt.blog_id AND t.name LIKE ' . $searchKey . '   ORDER BY b.cTime DESC LIMIT ' . $start . ',' . $num;
			} else { // 我喜欢的
				$sql = 'SELECT u.uname as uname,b.id,b.uid,b.cover,b.title,b.content,b.cTime,b.mTime,b.commentCount,b.republishCount,b.likeCount,b.terminal FROM  ts_blog b INNER JOIN  ts_user u ON b.uid=u.uid AND b.status=1 INNER JOIN ts_blog_comment c ON c.uid=b.uid WHERE c.mid=' . $mid . ' AND c.type=2  AND (b.title LIKE ' . $searchKey . ' OR b.content LIKE  ' . $searchKey . ' OR u.uname LIKE  ' . $searchKey . ' ) AND c.blog_id=b.id ORDER BY b.cTime DESC limit ' . $start . ',' . $num;
				$sqlTag = 'SELECT u.uname as uname,b.id,b.uid,b.cover,b.title,b.content,b.cTime,b.mTime,b.commentCount,b.republishCount,b.likeCount,b.terminal FROM ts_blog b  LEFT JOIN ts_user u   ON u.uid=b.uid AND  b.status=1 INNER JOIN ts_blog_comment c ON c.uid=b.uid  INNER JOIN ts_blog_tag bt ON bt.blog_id=b.id INNER JOIN ts_tag t ON bt.tag_id =t.tag_id AND t.name LIKE ' . $searchKey . '  AND c.mid=' . $mid . ' AND c.type=2 ORDER BY b.cTime DESC   LIMIT ' . $start . ',' . $num;
			}
		}
		$blogs = M ()->query ( $sql );
	 ...
	 * 根据标签获取博客
	 */
	public function getBlogListForTag($tag_id, $mid, $start = 0, $num = 5) {
		if ($tag_id == 2) {
			$querySql = 'SELECT u.uname as uname,b.* FROM ts_user u INNER JOIN  ts_blog b on u.uid=b.uid  INNER JOIN ts_blog_tag t on b.id = t.blog_id AND b.status=1 AND t.tag_id IN (SELECT tag_id FROM ts_tag t1 WHERE tag_hid=2) ORDER BY b.cTime desc LIMIT  ' . $start . ',' . $num;
		} else {
			$querySql = 'SELECT u.uname as uname,b.* FROM ts_user u INNER JOIN  ts_blog b on u.uid=b.uid  INNER JOIN ts_blog_tag t on b.id = t.blog_id AND b.status=1 AND t.tag_id = ' . $tag_id . ' ORDER BY b.cTime desc LIMIT ' . $start . ',' . $num;
		}
	 ...
	 * 获取课程中心博客
	 */
	public function getCourseCenterBlog($mid, $start = 0, $num = 5) {
		return $this->getBlogListForTag ( 2, $mid, $start = 0, $num = 5 );
	}
	/**
	 * 根据标签分类获取文章
	 */
	public function getTagCategoryBlog($condition, $start = 0, $num = 5) {
		$conditionSql = '';
		if ($condition ['time_type'] == 2) { // 今天
			$time = strtotime ( date ( 'Y-m-d' ) );
			$conditionSql = ' AND b.cTime>=' . $time;
		} else if ($condition ['time_type'] == 3) { // 最近一周
			$time = time () - 3600 * 24 * 7;
			$conditionSql = ' AND b.cTime>=' . $time;
		} else if ($condition ['time_type'] == 4) { // 最近一月
			$time = time () - 3600 * 24 * 30;
			$conditionSql = ' AND b.cTime>=' . $time;
		}
		if ($condition ['order_type'] == 1) { // 默认时间
			$conditionSql .= " ORDER BY b.cTime DESC";
		} else if ($condition ['order_type'] == 2) { // 评论数
			$conditionSql .= " ORDER BY b.commentCount DESC";
		} else if ($condition ['order_type'] == 3) { // 转载数
			$conditionSql .= " ORDER BY b.republishCount DESC";
		} else if ($condition ['order_type'] == 4) { // 喜欢
			$conditionSql .= " ORDER BY b.likeCount DESC";
		}
		$sql = 'SELECT u.uname as uname,b.* FROM ts_user u INNER JOIN  ts_blog b ON u.uid=b.uid AND b.status=1 INNER JOIN ts_blog_tag bt ON b.id=bt.blog_id AND bt.tag_id=' . $condition ['tag_id'] . $conditionSql . ' LIMIT ' . $start . ',' . $num;
		$blogs = M ()->query ( $sql );
		foreach ( $blogs as &$blog ) {
			$this->_convert ( $blog );
			
			$this->_queryCount ( $blog, $condition ['mid'] );
