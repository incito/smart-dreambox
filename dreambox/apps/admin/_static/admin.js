 ...
 * ui.error(L('PUBLIC_SELECT_DEPARTMENT')); return false; }
 */
 ...
 * 
 * @param integer
 *            type 要设置的话题类型 1:推荐 2:精华 3:锁定
 * @param integer
 *            topic_id 话题ID
 * @param integer
 *            value 话题现有的类型值，改为相反的。0变为1，1变为0
 */
 ...
 * 增加积分规则
 */
admin.addRule=function(){
	location.href = U('admin/CreditSetting/addRule');
}
/**
 * 删除积分规则
 */
admin.delRule = function(id){
    if("undefined" == typeof(id) || id=='')
        id = admin.getChecked();
    if(id==''){
        ui.error( '请选择规则' );return false;
    }  
    if(confirm( '确定删除选中规则？' )){
        $.post(U('admin/CreditSetting/delRule'),{id:id},function(msg){
            admin.ajaxReload(msg);
        },'json');
   }
};
admin.checkCredit=function(form){
	if(getLength(form.name.value)<1){
		ui.error("标识列不能为空");
		return false;
	}
 ...
 * 签到过期时间配置校验
 */
admin.configFeedbackCheck = function(form) {
	var day = form.expired_day.value;
	if (getLength(day) < 1) {
		ui.error("过期天数不能为空");
		return false;
	}
	if (isNaN(day) || day < 0) {
        ui.error("过期天数必须是大于等于0的整数");
        return false;
	}
 ...
 * 增加
 */
admin.add=function(url){
	location.href = url;
}
/**
 * 删除
 */
admin.del=function(id,item){
	var url=$('#delUrl').val();
	if("undefined" == typeof(id) || id=='')
	    id = admin.getChecked();
	if("undefined" == typeof(item) || item==''){
		item='项';
	}
	if(id==''){
		ui.error( '请选择要删除的'+item );return false;
	}  
	if(confirm( '确定删除选中'+item+'？' )){
...
admin.checkSchool=function(form){
	...
	if(getLength(form.school_number.value) < 1){
		 ui.error('学校编号不能为空');
	     return false;
	}
	if(getLength(form.email.value) < 1){
	...
	if(getLength(form.dream_number.value) < 1){
		 ui.error('学校序号不能为空');
	     return false;
	};
	var ret=true;
	var email=form.admin_email.value;
	if(getLength(email)>0){
		$.ajax({
			url:U('admin/DreamCenter/checkAdminEmail'),
			type:'post',
			async:false,
			data:'email='+email+'&id='+form.id.value,
...
admin.selectAdmin=function(obj,opt){
	ui.box.load(U('admin/DreamCenter/adminSelect')+'&sid='+(opt?obj:$('input[name=id]').val())+(opt?'&school=1':''),'选择梦想中心管理员');
}
admin.exportExcel=function(url){
	searchForm=$('#search_form form');
	if(searchForm.is(':hidden')){
		window.location.href=url;
	}else{
		exp=$("<input type='hidden' name='export' value='1' />");		
		var form=$("<form>");
		form.attr("style","display:none");
		form.attr("action",url);
		form.attr("method","post");
		form.append(searchForm.find('input,select').clone(false,true));
		form.append(exp);
		$("body").append(form);
		form.submit();
	}
