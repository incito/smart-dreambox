...
// 		$gradeInfo = model('System')->upgrade();
        ...
        $statistics[L('PUBLIC_DEV_TEAM')] = array(
        	L('PUBLIC_COPYRIGHT') => '<a href="http://www.incito.com.cn/" target="_blank">四川灵动信息技术有限公司</a>',
        );
	...
	private function wechatStat(){
	    ...
	    //
	    $onmap['activeTime'] = array('GT', time() - 1800);
	    $userInfo['onlineUser'] = count(D()->table('ts_online_wechat')->where($onmap)->findAll());
	    
	    $ymap['day'] = date('Y-m-d', strtotime("-1 day"));
	    $d = D('online_wechat_stats')->where($ymap)->find();
	    $userInfo['yesterdayUser'] = $d['most_online'];
	    
	    $ymap['day'] = array('GT', date('Y-m-d', strtotime("-7 day")));
	    $d = D('online_wechat_stats')->where($ymap)->field('max(most_online) AS most_online')->find();
	    $userInfo['weekAvg'] = $d['most_online'];
	    
	    $this->assign('wx_userInfo', $userInfo);
	    
	    $ymap['day'] = array('GT', date('Y-m-d', strtotime("-7 day")));
	    $d = D('online_wechat_stats')->where($ymap)->getHashList('day', '*');
	    ...
	    $visitCount['weekAvg'] = array('pv'=>ceil($apv/count($d)),'pu'=>ceil($apu/count($d)),'guest'=>ceil($agu/count($d)));
	    $this->assign('wx_visitCount', $visitCount);
