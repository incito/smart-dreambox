...
class ManageAction extends BaseAction {
	...
	public function do_update_album() {
		...
		// 检测相册是否已存在
		$find = $albumDao->getField('id', "userId={$this->mid} AND name='{$name}'");
		if($find) {
			$res['status'] = -1;
			$res['info'] = '相册已存在';
			exit(json_encode($res));
		...
		if (!$albumInfo) {
			$res['msg']="id='$albumId' AND userId='$this->mid' ";
			$res['status']=0;
			$res['info']='你没有权限编辑该相册！';
			exit(json_encode($res));
		}
		...
		// 如果相册隐私数据发生变化
		if ($album_privacy==4&&$albumInfo['privacy_data'] != $album_privacy_data) {
	...
	public function do_update_photo () {
		...
		//截取图片名字长度
		if($map['name']){
			$map['name']=mb_substr($map['name'], 0,15,'UTF-8'); 
		}else{
			//默认名字
			$map['name']='未命名';
		}
	...
	public function getAlbums(){
		//获取相册列表数据
		$data=	M('photo_album')->where("isDel=0 AND userId='".t($this->mid)."'")->field('id,name')->findAll();
		exit(json_encode($data));
	}
}