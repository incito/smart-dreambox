 ...
 */
function get_album_cover ($albumId, $album = '',$relationship, $width = 140, $height = 140) {
	
	// 隐私控制
	if ($_SESSION['mid']!= $album['userId']) {
		//自己可见
		if ($album['privacy'] == 3) {
			return 'self';
		//粉丝可见
		...
		//密码可见
			...
			// 如果密码不正确，则需要输入密码
			if ($cookie_password != md5($album['privacy_data'].'_'.$album['id'].'_'.$album['userId'].'_'.$_SESSION['mid'])) {
				return 'pwd';
			}
		}
	}
